...
import logoSrc from "@/assets/logo-clube-sax.png";
...
function drawCoverPage(doc: jsPDF, title: string, subtitle: string, badge: string, accentColor: [number, number, number], items: string[], logoBase64: string) {
  ...
  try {
    ...
    addText(doc, "CLUBEDOSAX", PAGE_W / 2, 28, { size: 12, color: COLORS.muted, style: "normal", align: "center" });
  }
  ...
  // Footer
  addText(doc, `\u00A9 ${new Date().getFullYear()} ClubedoSax \u2014 Todos os direitos reservados`, PAGE_W / 2, PAGE_H - 20, { size: 7, color: COLORS.muted, align: "center" });
...
function addPageNumber(doc: jsPDF, logoBase64: string) {
  ...
  for (let i = 2; i <= pageCount; i++) {
    ...
    addText(doc, "ClubedoSax", MARGIN, PAGE_H - 10, { size: 7, color: COLORS.muted });
  }
}
...
export async function generateDigitacaoPDF() {
  ...
  doc.save("Tabela-Digitacao-Completa-ClubedoSax.pdf");
}
...
export async function generateTransposicaoPDF() {
  ...
  addPageNumber(doc, logoBase64);
  doc.save("Kit-Transposicao-Instantanea-ClubedoSax.pdf");
}
...
export async function generateManutencaoPDF() {
  ...
  addPageNumber(doc, logoBase64);
  doc.save("Checklist-Manutencao-Sax-ClubedoSax.pdf");
}
...
export async function generateRotinaPDF() {
  ...
  const sections = [
    ...
    },
    {
      ...
      icon: "REPERTOIRE",
      items: [
        "Escolha 2-3 musicas do acervo ClubedoSax por semana",
  ...
  addPageNumber(doc, logoBase64);
  doc.save("Rotina-de-Estudo-Saxofonistas-ClubedoSax.pdf");
}
...
export async function generateTonalidadesPDF() {
  ...
  addPageNumber(doc, logoBase64);
  doc.save("Mapa-de-Tonalidades-Sax-ClubedoSax.pdf");
}
...
export async function generateMusicasPDF() {
  ...
  addPageNumber(doc, logoBase64);
  doc.save("100-Musicas-Saxofonista-ClubedoSax.pdf");
}
